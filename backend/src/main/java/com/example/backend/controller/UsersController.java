...
public class UsersController {
    ...
    @PreAuthorize("hasAnyRole('ADMIN','MANAGER')")
    @PostMapping("/invite")
    public ResponseEntity<UserDTO> inviteUser(@Valid @RequestBody InviteRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED).body(userService.inviteUser(request));
    }

    @PatchMapping("/me/name")
    public ResponseEntity<UserDTO> updateUserName(@Valid @RequestBody UpdateNameRequest req,
